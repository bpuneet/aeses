...
String query="select jira_user,jira_pass from APPLICATIONS where pentaho_user='"+user.getUsername()+"'";
int i=1;
...
<form  action="https://aesgse.onjira.com/secure/Dashboard.jspa" name="logonForm2" method="post"  target="_self">
  <input name="os_username" id="os_username" value="<%=request.getAttribute("os_username")%>" type="hidden" />
  <input name="os_password" id="os_password" type="hidden" value="<%=request.getAttribute("os_password")%>"/>
 </form>
</body>
</html>